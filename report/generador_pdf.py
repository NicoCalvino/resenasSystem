...
class GaugeSingle(Flowable):
    ...
    def draw(self):
        ...
        c.saveState()
        cx_offset = (CONTENT_W - self.gw) / 2
        c.translate(cx_offset, self.TRANSLATE_Y)
        g.canv = c
        g.draw()
        c.restoreState()
...
class InaceptableBadge(Flowable):
    """Badge rojo para reclamos clasificados como inaceptables."""
    def __init__(self):
        super().__init__()
        self.width  = 130
        self.height = 18
    ...
    def draw(self):
        c = self.canv
        c.setFillColor(C_RED)
        c.roundRect(0, 1, 126, 15, 4, stroke=0, fill=1)
        c.setFillColor(white)
        c.setFont("DJB", 8.5)
        c.drawCentredString(63, 5.5, "\u26a0  INACEPTABLE")
...
# ── FLOWABLE: WIDGET DE RECLAMOS (3 columnas horizontales) ────────────────────
class ReclaimosWidget(Flowable):
    """
    3 widgets distribuidos horizontalmente:
      Col 1 — Gauge RECLAMOS
      Col 2 — Thumb INACEPTABLES (OK / NOT OK)
      Col 3 — Índices CALIDAD / EQUIVOCADOS / INCOMPLETOS
    """
    GW       = 155   # ancho gauge/thumb, ajustado para 3 columnas
    GAUGE_CY = 4
    MOTIVO_CALIDAD    = "CALIDAD"
    MOTIVO_EQUIVOCADO = "EQUIVOCADO"
    MOTIVO_INCOMPLETO = "INCOMPLETO"

    def __init__(self, reclamos_count, tot, inac_count, reclamos):
        super().__init__()
    ...
    def _contar(self):
        ...
        for r in self.reclamos:
            razon = (r.get("razon") or "").strip().upper()
            if razon == self.MOTIVO_CALIDAD:    cal += 1
            if razon == self.MOTIVO_EQUIVOCADO: eq  += 1
            if razon == self.MOTIVO_INCOMPLETO: inc += 1
        return cal, eq, inc
    ...
    def draw(self):
        ...
        gw    = self.GW
        col   = self.col_w
        x_pad = (col - gw) / 2   # centra gauge/thumb dentro de su columna

        # ── Columna 1: Gauge RECLAMOS ─────────────────────────────────────────
        g = Gauge("RECLAMOS", self.reclamos_count, self.tot, w=gw)
        c.saveState()
        c.translate(x_pad, self.GAUGE_TRANSLATE_Y)
        g.canv = c
        g.draw()
        c.restoreState()

        # ── Columna 2: Índices CALIDAD / EQUIVOCADOS / INCOMPLETOS ────────────
        n = self.reclamos_count
        cal, eq, inc = self._contar()
        ...
        # ── Columna 3: Thumb INACEPTABLES ─────────────────────────────────────
        t = ThumbWidget(self.inac_count, self.reclamos_count, w=gw,
                        common_cy=self.COMMON_CY, thumb_r=32)
        c.saveState()
        c.translate(2 * col + x_pad, 0)
        t.canv = c
        t.draw()
        c.restoreState()


# ── FLOWABLE: GAUGE + ÍNDICES DE RECLAMOS (legacy, no usado) ──────────────────
...
def build_story(data, styles, tpr):
    ...
    # Widget: gauge RECLAMOS (izq) + thumb INACEPTABLES (der)
    ...
    if not reclamos:
        ...
        for rc in reclamos_ord:
            block = []
            inaceptable = rc.get("inaceptable", False)
            ...
            # Badge INACEPTABLE si corresponde
            if inaceptable:
                block.append(InaceptableBadge())
            ...
            sep_color = C_RED_M if inaceptable else HexColor("#FFCC88")
    ...
    for grupo in grupos:
        ...
        for idx, plato in enumerate(grupo["platos"]):
            ...
            for tag in plato["tags"]:
                block.append(Paragraph(tag, styles["tag"]))
        ...
        block.append(HRFlowable(width="100%", thickness=1.2,
                                 color=HexColor("#AAAAAA"), spaceAfter=3.5 * mm))
        story.append(KeepTogether(block))
...
SAMPLE_DATA = {
    ...
    "reclamos": [
        ...
        {"fecha": "31/03/2026 12:31", "orden": "460099253", "marca": "Green Eat",
         ...
         "razon": "INCOMPLETO",
         "comentario": "No llegó el salmón, encontré un pelo dentro del poke",
         "inaceptable": True},
