...
def _buscar_tienda_peya_por_id(partner_id: str) -> dict | None:
    """Busca tienda directamente por py_id (lookup O(1))."""
    try:
        pid = int(partner_id)
    except (ValueError, TypeError):
        return None
    return PY_INDEX.get(pid)
...
PEYA_WEBHOOK_MOTIVO: dict[str, str] = {
    "missing item":  "INCOMPLETO",
    "wrong item":    "EQUIVOCADO",
    "wrong order":   "EQUIVOCADO",
...
def parsear_reclamos_peya_webhook(
    ...
    Columnas del CSV:
      partner_id        → py_id de la tienda (lookup directo; columna nueva)
      order_id_general  → orden_id
      partner_name      → nombre del restaurante (fallback si no hay partner_id)
    ...
    Formato del nuevo webhook: cuando una orden tiene más de un plato, las filas
    siguientes repiten todos los datos de la orden con únicamente la columna
    `producto` distinta. Se agrupa por `order_id_general` acumulando productos;
    como no es posible determinar cuál fue el plato reclamado, platos_reclamados
    se deja en blanco.
    """
    ruta = Path(ruta_csv)
    if not ruta.exists():
    ...
    def _finalizar_orden(pending: dict) -> None:
        ...
        # Tienda — primero por partner_id (lookup directo), luego por nombre
        tienda = _buscar_tienda_peya_por_id(pending["partner_id"]) if pending["partner_id"] else None
        if not tienda:
            tienda = _buscar_tienda_peya(pending["partner_name"])
        if not tienda:
            sin_tienda.append(pending["partner_name"] or pending["partner_id"])
            logger.warning(
                f"PedidosYa reclamos fila {pending['fila']}: "
                f"tienda no encontrada — partner_id='{pending['partner_id']}' "
                f"partner_name='{pending['partner_name']}'"
            )
            return
    ...
    with open(ruta, encoding="utf-8-sig", errors="replace") as f:
        ...
        for i, row in enumerate(reader, 2):
            ...
            # Saltar filas sin order_id ni identificador de tienda
            if not order_id_raw and not partner_id and not partner_name:
                continue

            # ── Fila de continuación (mismo order_id que la orden en curso) ─
            # El nuevo webhook repite todos los datos en cada fila, cambiando
            # sólo el producto. Se agrupa por order_id_general.
            # Se compara con strip() para tolerar espacios o variaciones de encoding.
            if pending and order_id_raw and order_id_raw.strip() == pending["orden_id"].strip():
                plato_extra = col("producto")
                if plato_extra and plato_extra not in pending["productos"]:
...
def api_resenas_peya(token: str, vendor_codes: list[str],
    ...
    # La API usa endDate como exclusivo (T03:00Z = medianoche Argentina).
    # Se suma 1 día a hasta para que el rango sea inclusivo en ambos extremos.
    # Ej: desde=27/4, hasta=27/4 → endDate=28/4T03Z (cubre todo el 27/4).
    #     desde=27/4, hasta=28/4 → endDate=29/4T03Z (cubre 27/4 y 28/4).
    body = {
        ...
        "filter": {
            ...
            "endDate":   _diso(hasta + timedelta(days=1)),
        },
    }
    try:
...
def _parsear_fecha_peya(s: str) -> datetime:
    # La API devuelve UTC; se convierte a hora local Argentina (UTC-3)
